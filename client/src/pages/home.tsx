...
export default function Home() {
  ...
  const [selectedDates, setSelectedDates] = useState<Record<string, { from: string; to: string; quantity?: number }>>({});
  ...
  const availabilityMutation = useMutation({
    mutationFn: async ({ equipmentId, dateFrom, dateTo }: { equipmentId: string; dateFrom: string; dateTo: string }) => {
      const response = await apiRequest("POST", `/api/equipment/${equipmentId}/availability`, {
        dateFrom,
        dateTo,
      });
  ...
  const handleConfirmDates = async (dateFrom: string, dateTo: string, quantity: number) => {
    try {
  ...
  const handleAddToCart = (equipmentId: string) => {
    const equipmentItem = equipment.find(e => e.id.toString() === equipmentId);
  ...
  const handleSubmitReservation = (data: ContactForm & { cartItems: CartItem[] }) => {
  ...
  return (
    <div className="min-h-screen bg-neutral-light">
      ...
      <HeroBanner onScrollToEquipment={scrollToEquipment} />
            ...
            <div className="flex-1">
              {isLoading ? (
                <div className="space-y-4">
                  {[...Array(6)].map((_, i) => (
                    <div key={i} className="bg-gray-200 animate-pulse rounded-xl h-48 w-full" />
                  ))}
                </div>
              ) : (
      ...
      <CalendarModal
        ...
        onAddToCart={async (dateFrom, dateTo, quantity) => {
          const equipmentItem = equipment.find(e => e.id.toString() === currentEquipmentId);
          ...
          try {
            ...
            // Check if adding this quantity would exceed available stock
            const currentCartQuantity = cartItems
              .filter(item => item.id === currentEquipmentId)
              .reduce((total, item) => total + item.quantity, 0);
